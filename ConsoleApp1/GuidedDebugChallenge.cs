...
public static class GuidedDebugChallenge
{
    public static void DebugChallenge()
    {
        ...
        while (transactions > 0)
        {
            ...
            int paymentFives =
                (itemCost % 10 > 7) ? 1 : 0; // value is 1 when itemCost ends with 8 or 9, otherwise value is 0
            int paymentTens =
                (itemCost % 20 > 13)
                    ? 1
                    : 0; // value is 1 when 13 < itemCost < 20 OR 33 < itemCost < 40, otherwise value is 0
            ...
            try
            {
                ...
                Console.WriteLine($"Transaction Succeffuly Completed.");
        ...
        static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
        {
            ...
            if (CantGiveChange(cashTill, changeNeeded, twenties, tens, fives, ones))
            {
        ...
        // this is the provided solution but still has invalid till amounts at the end
        ...
        static bool CantGiveChange(int[] cashTill, int changeDue, int twenties, int tens = 0, int fives = 0, int ones = 0)
        {
            int[] tempCashTill = (int[]) cashTill.Clone();
            tempCashTill[3] += twenties;
            tempCashTill[2] += tens;
            tempCashTill[1] += fives;
            tempCashTill[0] += ones;
            
            while ((changeDue > 19) && (tempCashTill[3] > 0))
            {
                tempCashTill[3]--;
                changeDue -= 20;
            }

            while ((changeDue > 9) && (tempCashTill[2] > 0))
            {
                tempCashTill[2]--;
                changeDue -= 10;
            }

            while ((changeDue > 4) && (tempCashTill[1] > 0))
            {
                tempCashTill[1]--;
                changeDue -= 5;
            }

            while ((changeDue > 0) && (tempCashTill[0] > 0))
            {
                tempCashTill[0]--;
                changeDue -= 1;
            }
    ...
    public static void DebugChallengeSolution()
    {
        string? readResult = null;
        bool useTestData = false;
